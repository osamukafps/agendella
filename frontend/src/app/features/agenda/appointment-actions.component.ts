  ...
  template: `
    <div class="actions-row">
      @if (canComplete(appt.status)) {
        <button class="action-btn action-btn--success" (click)="completed.emit()">Concluir</button>
      }
      @if (canNoShow(appt.status)) {
        <button class="action-btn action-btn--warning" (click)="noShowed.emit()">Não veio</button>
      }
      @if (canReschedule(appt.status)) {
        <button class="action-btn action-btn--neutral" (click)="rescheduled.emit()">Reagendar</button>
      }
      @if (canCancel(appt.status)) {
        <button class="action-btn action-btn--danger" (click)="cancelled.emit()">Cancelar</button>
      }
      @if (canResolveReview(appt.requiresReview)) {
        <button class="action-btn action-btn--review" (click)="reviewResolved.emit()">Resolver revisão</button>
      }
    </div>
